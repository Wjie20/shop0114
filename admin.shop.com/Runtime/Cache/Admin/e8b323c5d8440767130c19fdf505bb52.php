...
<input type="button" value="隐藏" class="button ajax-post" url="<?php echo U('changeStatus',array('status'=>0));?>"/>
    ...
    <div class="list-div" id="listDiv">
        <table cellpadding="3" cellspacing="1">
            ...
            <?php if(is_array($rows)): $i = 0; $__LIST__ = $rows;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$row): $mod = ($i % 2 );++$i;?><tr style="text-align: center">
                   ...
                   <td class='center'><?php echo ($row["site_url"]); ?></td><td class='center'><img src="http://shop-brand-logo.b0.upaiyun.com/<?php echo ($row["logo"]); ?>!50" /></td><td class='center'><?php echo ($row["intro"]); ?></td><td class='center'><?php echo ($row["sort"]); ?></td><td align='center'><a class='ajax-get' href='<?php echo U('changeStatus',array('id'=>$row['id'],'status'=>(1-$row['status'])));?>'><img src='http://admin.shop.com/Public/Admin/images/<?php echo ($row["status"]); ?>.gif'/></a></td>
